...
void DebugDraw::drawSolidCircle(b2Transform xf, float radius, b2HexColor color, void* context) {
  ...
  // Create center point with proper initialization
  b2Vec2 center = b2TransformPoint(xf, { 0.0f, 0.0f });

  const int segments = 16;
  std::vector<float> vertexData;
  vertexData.reserve((segments + 1) * 6); // 6 floats per vertex (pos + color)
